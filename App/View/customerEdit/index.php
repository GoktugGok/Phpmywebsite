...
<html lang="en">
  ...
  <main>
    <div class="container py-4 ">
      ...
      <div class="p-5 mb-4 bg-light rounded-3">
        <form class=" row g-3 m-auto rounded-2 " style="width: 100%;" id="muşteriguncelle">
          <input type="hidden" id="muşterid" value="<?=$data['update']['id']?>">
          <div class="col-12 text-center"><h1>Müşteri Ekle</h1></div>
          <div class="col-md-6">
            <label for="name" class="form-label">Name</label>
            <input type="text" class="form-control" id="name" value="<?=$data['update']['name']?>">
          </div>
          <div class="col-md-6">
            <label for="surname" class="form-label">Surname</label>
            <input type="text" class="form-control" id="surname" value="<?=$data['update']['surname']?>" >
          </div>
          <div class="col-md-6">
            <label for="company" class="form-label">Company</label>
            <input type="text" class="form-control" id="company" value="<?=$data['update']['company']?>">
          </div>
          <div class="col-md-6">
            <label for="email" class="form-label">Email</label>
            <input type="email" class="form-control" id="email" value="<?=$data['update']['email']?>">
          </div>
          <div class="col-md-12">
            <label for="phone" class="form-label">Phone</label>
            <input type="text" pattern="[0-9]{10}" title="Telefon numaranızı doğru giriniz" maxlength="10" class="form-control" id="phone" value="<?=$data['update']['phone']?>" >
 ...
 <script>
    const muşteriguncelle = document.getElementById('muşteriguncelle');

    muşteriguncelle.addEventListener('submit',(e)=>{
      let name = document.getElementById('name').value;
      let surname = document.getElementById('surname').value;
      let email = document.getElementById('email').value;
      let company = document.getElementById('company').value;
      let phone = document.getElementById('phone').value;
      let muşterid = document.getElementById('muşterid').value;
      ...
      formData.append('muşterid',muşterid);
